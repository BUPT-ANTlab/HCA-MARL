...
class Replay_buffer():
    ...
    def sample(self, batch_size):
        ...
        for i in ind:
            ...
            x.append(dc(X))
            y.append(dc(Y))
            u.append(np.array(dc(U), copy=False))
            r.append(np.array(dc(R), copy=False))
            d.append(np.array(dc(D), copy=False))
        global_state = self.merge_dicts(x)
        global_state_next = self.merge_dicts(y)
        return global_state, global_state_next, np.array(u), np.array(r).reshape(-1, 1), np.array(d).reshape(-1, 1)
